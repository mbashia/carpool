defmodule Carpool.Trips do
  ...
  def search(search) do
    query = from t in Trip, where: fragment("? LIKE ?", t.from, ^"%#{search}%" )or fragment("? LIKE ?", t.to, ^"%#{search}%" )or fragment("? LIKE ?", t.departure_time, ^"%#{search}%" )or fragment("? LIKE ?", t.return_time, ^"%#{search}%" )
    Repo.all(query)
  end
